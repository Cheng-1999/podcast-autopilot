...
- `python -m podcast_autopilot plan-fillers <audio> [--model small|medium]`：
  ...
  `filler_min_probability`（預設 0.5）」才算數。偵測到的候選會以
  `kind: "filler"`、`enabled: false` 的形式寫進 `out/<檔名>/plan.json`
  （純提案，不會改變輸出，除非人工把 `enabled` 改成 `true`）。多字語助詞
...
且每個 `filler` 的範圍必須完全落在某個 `keep` 項目內。
...
`audit.py` requires every `filler` item to sit fully inside a `keep` item and
