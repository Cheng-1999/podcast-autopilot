...
export const NewEpisodePage: React.FC = () => {
  ...
  const [duck, setDuck] = useState({ threshold: "0.03", ratio: "8", attack: "5", release: "300" });
  ...
  async function submit(runNow: boolean) {
    ...
    setSaving(true); try { const body: EpisodeCreateBody = { title: title.trim(), episode, parts: parts.map((p) => p.path), intro: intro?.path || null, outro: outro?.path || null, bgm: bgm ? { path: bgm.path, gain_db: -18, duck: Object.fromEntries(Object.entries(duck).map(([k, v]) => [k, Number(v)])) } : null, chapters: chapters.filter((c) => c.title || c.start), tags: { ...tags, year: tags.year ? Number(tags.year) : null } }; const created = await createEpisode(body); if (runNow) await runEpisode(created.id, { profile: "default", model: "medium", force: false, skip: [] }); navigate(`/episodes/${created.id}`); } catch (e) { setError((e as Error).message); } finally { setSaving(false); }
  }
  ...
  return <div className="wizard-page">
    ...
    {step === 3 && <section className="wizard-panel"><h2>配樂與章節 <small>全部為選填</small></h2><div className="asset-grid">{([["intro", "Intro", intro, setIntro], ["outro", "Outro", outro, setOutro], ["bgm", "BGM", bgm, setBgm]] as const).map(([key, name, value]) => <div className="asset-box" key={key}><div className="asset-heading"><b>{name}</b><span>{value?.name || "尚未加入"}</span></div><UploadDrop label={`加入 ${name} 音檔`} onFiles={(files) => addFiles(files, key as "intro" | "outro" | "bgm")} />{key === "bgm" && <div className="duck-grid">{field("GAIN dB", "-18", () => {})}{field("THRESHOLD", duck.threshold, (v) => setDuck({ ...duck, threshold: v }))}{field("RATIO", duck.ratio, (v) => setDuck({ ...duck, ratio: v }))}{field("ATTACK ms", duck.attack, (v) => setDuck({ ...duck, attack: v }))}{field("RELEASE ms", duck.release, (v) => setDuck({ ...duck, release: v }))}</div>}</div>)}</div><div className="chapter-heading"><h3>章節 CHAPTERS</h3><span className="mono">總長 {formatDuration(totalDuration)}</span></div><div className="chapter-list">{chapters.map((chapter, index) => <div className="chapter-row" key={index}><span className="mono">{String(index + 1).padStart(2, "0")}</span><input className="dense-input chapter-time" placeholder="mm:ss" value={chapter.start} onChange={(e) => updateChapter(index, "start", e.target.value)} /><input className="dense-input" placeholder="章節標題" value={chapter.title} onChange={(e) => updateChapter(index, "title", e.target.value)} /><button className="dense-btn" onClick={() => setChapters((old) => old.filter((_, i) => i !== index))}>移除</button></div>)}</div><button className="dense-btn" onClick={() => setChapters((old) => [...old, { start: "", title: "" }])}>＋ 新增章節</button><div className="wizard-actions"><button className="dense-btn" onClick={() => setStep(2)}>← 上一步</button><button className="dense-btn" disabled={saving} onClick={() => submit(false)}>建立集數</button><button className="dense-btn primary" disabled={saving} onClick={() => submit(true)}>{saving ? "建立中..." : "建立並立即執行 →"}</button></div></section>}
  </div>;
};
