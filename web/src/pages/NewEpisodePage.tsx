...
export const NewEpisodePage: React.FC = () => {
  ...
  return <div className="wizard-page">
    <header className="page-header"><div><button className="dense-btn" onClick={() => navigate("/episodes")}>{t("nav.list")}</button><h1>{t("new.heading")} <span className="mono">/ NEW EPISODE</span></h1></div><span className="mono wizard-counter">{t("new.step")} {step} / 3</span></header>
    <nav className="wizard-steps">{[t("new.basic"), t("new.parts"), t("new.assets")].map((name, i) => <button key={name} className={step === i + 1 ? "active" : step > i + 1 ? "complete" : ""} onClick={() => setStep(i + 1)}><b>{String(i + 1).padStart(2, "0")}</b>{name}</button>)}</nav>
