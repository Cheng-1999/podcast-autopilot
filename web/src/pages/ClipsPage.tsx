...
import { createJobEventSource, deleteClips, fetchClips, fetchEpisode, generateClips } from "../api/client";
...
export const ClipsPage: React.FC = () => {
  ...
  const [loadingClipId, setLoadingClipId] = useState<string | null>(null); const [playError, setPlayError] = useState(""); const [deleting, setDeleting] = useState(false);
  ...
  async function removeClips() { if (!window.confirm(t("clips.deleteConfirm"))) return; setDeleting(true); try { await deleteClips(id, activePart); await queryClient.invalidateQueries({ queryKey: ["clips", id, activePart] }); setMessage(t("clips.deleted")); } catch (e) { setMessage((e as Error).message); } finally { setDeleting(false); } }
